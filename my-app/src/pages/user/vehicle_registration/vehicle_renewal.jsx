...
export default function RenewalForm() {
  ...
  const handleSubmit = async () => {
    ...
    // Check if there are any validation errors or missing files
    ...
    try {
      // Fetch user data
      const res = await axios.get(`http://localhost:8080/applicants/get-by-email/${email}`, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });
  
      // Update registrationData state with fetched user data
      const userData = res.data; // Adjust based on actual response structure
      setRegistrationData({
        surname: userData.lastName,
        givenname: userData.firstName,
        mi: userData.middleInitial,
        sname: userData.studentName,
        idno: userData.idNumber,
        yearlevel: userData.gradeLevel,
        contactno: userData.contactNumber,
        vmake: userData.vehicleMake,
        plateno: userData.plateNo,
        color: userData.color,
        isFourWheel: userData.vehicleType,
        stickerType: userData.isParking,
        address: userData.address, // Make sure this matches your response structure
      });
      ...
      if (res.status === 200 && res2.status === 200 && res3.status === 200) {
        setSnackbarMessage("Renewal successful");
        setSnackbarOpen(true);
        setModalOpen(true); 
      } else {
